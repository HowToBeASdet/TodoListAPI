...
public class FilesStorageServiceImpl implements FilesStorageService {
    ...
    public void save(String userId, MultipartFile file) {
        try {
            Path files = root.resolve(String.valueOf(userId));
            ...
            Files.copy(file.getInputStream(), this.root.resolve(String.valueOf(userId)));
        } catch (Exception e) {
    ...
    public void delete(String userId) {
        ...
        try {
            resource = new UrlResource(files.toUri());
            if (resource.exists() || resource.isReadable()) {
                Files.deleteIfExists(files);
            }
        } catch (MalformedURLException e) {
